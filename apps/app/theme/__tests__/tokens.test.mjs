...
test("tokens.json has the expected top-level groups", () => {
  const requiredGroups = ["color", "font", "size", "radius", "spacing", "shadow"];
  for (const g of requiredGroups) {
    assert.ok(Object.keys(json).includes(g), `tokens.json missing required group "${g}"`);
  }
  // Canonical tokens.json also ships an `alias` semantic layer and a
  // `themes` map (sepia/night/day + dusk). They are extras the typed
  // primitive shape doesn't have to declare, so we just spot-check they're
  // present in the JSON to prevent silent drift.
  for (const optional of ["alias", "themes"]) {
    assert.ok(Object.keys(json).includes(optional), `tokens.json missing canonical "${optional}" layer`);
  }
});
...
test("font, size, radius, spacing, shadow keys are stable", () => {
  // `serif` is an optional canonical face (used by reader-serif default Lora
  // stack). Required faces are body/display/mono/ui.
  const fontKeys = Object.keys(json.font).sort();
  for (const required of ["body", "display", "mono", "ui"]) {
    assert.ok(fontKeys.includes(required), `tokens.json font.${required} missing`);
  }
  assert.deepEqual(
