...
The LLM router supports Anthropic as quality-first, Vertex/Gemini fallback, DeepSeek for low-risk cost-optimized text, and deterministic fallback. Provider outputs are parsed and safety-checked before persistence. Pro media orchestration is asynchronous and does not block text reading.
