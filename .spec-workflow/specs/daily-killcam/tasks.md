...
**Sequencing note (honesty risk).** The label-bucketing premise — that
independently LLM-generated choice labels for the same daily opening
"rhyme" enough to bucket together — is EMPIRICAL, not guaranteed. Land R1
recording + the rewind deletion (tasks 2.1 and the recording half of 4.1)
FIRST and let a real Daily accrue live rows, so task 4.2's bucket-spread
measurement (distinct `choiceKey`s per turn) reads off actual data before
the pulse UI (task 3.2) is committed to readers. If the spread is too wide
to be meaningful, the turn-cap / normalization gets tuned — or the surfaces
stay dark — before anyone ships a chip that claims "62% of readers" over
buckets that don't actually cohere.

---

## Agent DK-PURE — owns `convex/daily.ts` + `convex/tests/daily.test.ts`

- [x] 1.1 Choice-key derivation + constants + phrase table (R1.2, R2.4, R4.1)
...
- [x] 1.2 `computeChoicePulse` aggregation (R2.1, R2.2, R2.3)
...
## Agent DK-CLIENT — owns `apps/app/lib/dailyApi.ts`, `apps/app/lib/gameApi.ts` (the `RemoteScene` type, for the `dailyId` widening), `apps/app/components/daily/**`, ReaderScreen mount, client tests

- [x] 3.1 Transport + render model (R2.3, R3.4)
...
- [x] 3.2 `DailyPulseChip` + `OpeningForks` surfaces (R3.1, R3.2)
  ...
  Both self-hide with zero layout shift. The `RemoteScene` type in
  `apps/app/lib/gameApi.ts` (grep `type RemoteScene` ≈ :185; imported by
  `useTurn.ts` at :31) gains the optional `dailyId` field (adapter side of
  2.2's widening). Node/vitest tests for the join/hide logic.
  - _Leverage: `apps/app/components/reading/ReaderScreen.tsx:415-426` (ThreadsPill mount + self-hide pattern), `apps/app/components/daily/DailyResults.tsx`, `apps/app/lib/gameApi.ts` grep `type RemoteScene` ≈ :185 (the type to widen; useTurn.ts imports it), `apps/app/hooks/useTurn.ts` (choice history labels)_
...
- [x] 4.1 Schema + game.ts call sites + lifecycle (R1.1, R1.4, BC7, DK1)
  Add `daily_choice_results` per design §1.1 (four indexes, including the
  `by_accountId` purge index settled in §1.1/§5). Wire
  `recordDailyChoiceIfEligible` at the SINGLE live llm-driven turn_history
  site (`runLlmDrivenBeginStreaming`, `game.ts:4340`) — NOT the dead
  `runLlmDrivenSubmitChoice` (`:4855`, `@deprecated DEAD CODE`, DK1) — and
  `deleteDailyChoicesFromTurn` in the rewind mutation (`rewindSaveTurns`,
  `game.ts:1086`, turn_history delete loop ~`:1148`). Add the `deleteAccount`
  purge (via `by_accountId`) + `exportAccount` bundle lines
  (`accountFunctions.ts:154`, `:124`). Then: full monorepo typecheck + all
  suites + dockerized deploy as the merge gate.
  - _Leverage: `convex/schema.ts:481` (daily_results template), `runLlmDrivenBeginStreaming` (`convex/game.ts:4340`, the single live DK1 site), `rewindSaveTurns` (`convex/game.ts:1086`), `convex/accountFunctions.ts:154-216` (deleteAccount deleteByIndex pattern)_
  - _Requirements: R1.1, R1.4, R1.5, NFR Security_
  - _Prompt: Implement the task for spec daily-killcam, first run spec-workflow-guide to get the workflow guide then implement the task: Role: integrator with sole ownership of reserved files and game.ts. Task: land the daily_choice_results table, wire both recording call sites plus the rewind deletion, and close the account-lifecycle loop per design sections 1, 2, and 5. Restrictions: additive edits only to reserved files; recording wires ONLY into the live streaming-begin site (runLlmDrivenBeginStreaming, game.ts:4340) — the non-streaming runLlmDrivenSubmitChoice is dead code, do NOT wire it (DK1); nothing merges until typecheck + full suites are green. Success: dockerized deploy healthy; a fresh daily run writes rows for turns 1–3 and a rewind removes them. Mark [-] when starting, log-implementation when done, then [x]._

- [ ] 4.3 FOLLOW-UP — OpeningForks results-strip needs a reader's-own-labels source (R3.2)
  Implementation gap found at wire-up: `DailyResults` renders ONLY on the
  standalone route `apps/app/app/daily/[dailyId]/index.tsx` (reached from the
  DailyCard's already-played state), a fresh navigation with NO `useTurn`
  session — so the reader's own early choice LABELS (which `OpeningForks`
  joins against `pulses`) are not in scope there. R3.1's in-run
  `DailyPulseChip` is fully wired (server `dailyId` → `RemoteScene` →
  `ReaderProjection.dailyId` → chip); only the R3.2 results-screen recap is
  dark. Cleanest fix (spoiler-safe, all the reader's OWN data): extend
  `getChoicePulse` to also return the reader's own `saveId` (`readerSaveId`,
  from their own `daily_choice_results` rows — BC10-clean), then have the
  results route fetch `getRemoteRunHistory(readerSaveId)` and pass turns 1–3
  `choiceLabel`s as `openingChoices` to `DailyResults`. Touches
  `convex/dailyFunctions.ts` (getChoicePulse shape), `apps/app/lib/dailyApi.ts`
  (adapter + wire type), and the route. Until then the strip self-hides (BC9,
  harmless). Alternatively, render `DailyResults` inline at daily end-of-run
  inside `ReaderScreen` where `choiceHistory` already exists.
  - _Requirements: R3.2_
