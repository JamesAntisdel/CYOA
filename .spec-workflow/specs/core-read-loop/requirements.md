...
### Requirement 9 — LLM is structurally prevented from mutating state
...
1. WHEN Convex calls the LLM for a turn THEN the prompt SHALL provide the engine-computed current state summary, authored scene seed, and valid next-choice candidates, and SHALL instruct the model to write coherent prose around those engine-owned facts without choosing or mutating game state.
2. WHEN parsing LLM output THEN Convex SHALL reject any output field naming a stat-delta, an inventory item add/remove, a flag set/clear, or a node transition. Specifically, the parsed shape SHALL contain ONLY: prose text, optional prose-anchor metadata for already-computed engine events, and optional presentation metadata for engine-provided choices.
3. WHEN LLM output references a stat name, item name, or flag that does not exist in the engine's registry for the current story THEN Convex SHALL strip or sanitize the reference before persisting; the engine SHALL be the only source of state mutations.
4. IF the LLM produces text that attempts to assert state changes ("…and you now have 999 gold") THEN that text SHALL be allowed in narrative prose ONLY if the engine has independently produced a matching state delta; otherwise the player's actual state shown in the HUD SHALL contradict the prose, which is acceptable as a soft signal but the **state itself SHALL never reflect the LLM's assertion**.
...
- **LLM output is untrusted text**: parsed only against a strict Zod schema; rendered as plain text (no HTML, no `dangerouslySetInnerHTML`); engine state mutations are produced exclusively by the engine.
