...
- [ ] 53. Engine: NPC state, effects, and migration
...
- [ ] 54. Server: NPC context surfaces in scene prompts
  - Files: `convex/llm/prompts/scene.ts`, `convex/llm/types.ts`, `convex/game.ts` (the request-building sites — same ones touched by the seed-flow change), `convex/tests/*.test.ts`
...
- [ ] 56. Choice schema: requiresNpc / targetNpc / skill_check includeCompanions
  - Files: `packages/engine/src/choices.ts` (or wherever choice evaluation lives — verify), `packages/engine/src/effects.ts`, `packages/engine/tests/engine.test.ts`
